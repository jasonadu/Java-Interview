 ...
 * 3、创建的对象需要消耗的资源过多，比如 I/O 与数据库的连接等。
 * 
 * 注意事项：
 ...
 * 实现难度：易
 * 
 * 描述：这种方式是最基本的实现方式，最大的问题就是不支持多线程。
 ...
 * 优点：没有加锁，执行效率会提高。
 * 缺点：类加载时就初始化，浪费内存。
 * 它基于 classloader 机制避免了多线程的同步问题，不过，instance 在类装载时就实例化（多个类加载器也可能生成多个实例）
 ...
 * 如果不使用volatile修饰变量的话，有可能出现实例未初始化完成的问题，
 * 解决办法使用volatile轻量级锁。保证happens-before,读发生在写之后。
 * 
 * public class Singleton {  
 ...
 * 它跟第3种方式不同的是：第3种方式只要 Singleton 类被装载了，那么 instance 就会被实例化
